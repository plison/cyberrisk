...
class Train(object):
    ...
    def add_subparser(self, name, parser):
        ...
        #subparser.add_argument("--patience", default=None, type=int)
    ...
    def __call__(self, config):
        ...
        vocab= Vocab.from_corpus(corpus=train, min_freq=3)
        if config.fembed:
        ...
        trainset = TextDataset(X)#vocab.numericalize(train))#X)
        ...
        for epoch in range(1, config.epochs + 1):
            ...
            print(f"{'train:':6} Loss: {loss:.4f} \t f1: {train_metric:.2f}")
            loss, dev_metric = model.evaluate(dev_loader, config.punct)
            print(f"{'dev:':6} Loss: {loss:.4f} \t f1: {dev_metric:.2f}")
