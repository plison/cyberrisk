...
status2simplified = {"probably_exploited": "could_be_exploited",
                     "could_be_exploited": "could_be_exploited",
                     "unlikely_to_be_exploited": "unlikely_to_be_exploited",
                     "probably_not_exploited": "unlikely_to_be_exploited",
                     "unclear": "unclear"}
...
def convert(document):
    ...
    for entity in document["entities"]:
        if entity["span_text"].startswith("CVE-"):
            ...
            stats = status2simplified[status]
...
train_ind = ceil(len(labelled_corpus) * 0.8) - 1 - 8
test_ind = ceil(len(labelled_corpus) * 0.1) + train_ind - 1
...
labelled_corpus, unlikely_instances = split_on_label(labelled_corpus, "unlikely_to_be_exploited")
print("unlikely", len(unlikely_instances))
print("without", len(labelled_corpus))
#test = [unclear_instances[0]]
#train = [unclear_instances[1]]
labelled_corpus += unlikely_instances[:50]
random.shuffle(labelled_corpus)
