...
def chuliu_edmonds(scores):
  ...
  #scores = np.insert(scores, 0, 0., axis=0)
  scores[0] = 0
  scores[0,0] = 1.
  #print(scores)
  #exit()
...
def chuliu_edmonds_one_root(scores):
  ...
  #roots_to_try = np.where(np.equal(tree, 0))[0]+1
  if len(roots_to_try) == 1:
    tree = np.delete(tree, 0, axis=0)
    return tree
  ...
  if len(roots_to_try) == 0:
      ...
      #roots_to_try = np.where(scores[:,0] >= 1/len(scores))[0]+1
  # *sigh* just grab the most likely one
  if len(roots_to_try) == 0:
      roots_to_try = np.array([np.argmax(scores[1:,0])+1])
      #roots_to_try = np.array([np.argmax(scores[:,0])+1])
  ...
  try:
    ...
    raise
  best_tree = np.delete(best_tree, 0, axis=0)
  return best_tree
